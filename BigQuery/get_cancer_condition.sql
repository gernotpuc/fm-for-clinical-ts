-- 1️⃣ Base cohort (same as before)
WITH fever AS (
...
cancer_diag AS (
  SELECT DISTINCT hadm_id, subject_id, icd_code
...
adult_cohort AS (
  ...
  #JOIN cancer_diag c USING (hadm_id, subject_id)
  JOIN cancer_diag c USING (subject_id)
...
-- 2️⃣ Cancer condition flags per admission
cancer_flags AS (
  SELECT
    hadm_id,
    subject_id,
    MAX(CASE
          WHEN REGEXP_CONTAINS(icd_code, r'^(200|201|202|203|204|205|206|207|208|C8[1-9]|C9[0-6])')
            THEN 1 ELSE 0 END) AS cond_hemat,
    MAX(CASE
          WHEN REGEXP_CONTAINS(icd_code, r'^(204|205|206|207|208|C9[1-5])')
            THEN 1 ELSE 0 END) AS cond_leuk,
    MAX(CASE
          WHEN REGEXP_CONTAINS(icd_code, r'^(14[0-9]|15[0-9]|16[0-9]|17[0-9]|18[0-9]|19[0-9]|C0[0-9]|C1[0-9]|C2[0-9]|C3[0-9]|C4[0-9]|C5[0-9]|C6[0-9]|C7[0-9]|C80)')
            THEN 1 ELSE 0 END) AS cond_solid
  FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd`
  GROUP BY hadm_id, subject_id
)

-- 3️⃣ Combine with cohort
SELECT
  ac.hadm_id,
  ac.subject_id,
  COALESCE(cf.cond_hemat, 0) AS cond_hemat,
  COALESCE(cf.cond_solid, 0) AS cond_solid,
  COALESCE(cf.cond_leuk, 0)  AS cond_leuk
FROM adult_cohort ac
LEFT JOIN cancer_flags cf
  ON ac.hadm_id = cf.hadm_id
ORDER BY ac.hadm_id;
