...
-- 2️⃣ Find first antibiotic start time for each hadm_id
first_abx AS (
  SELECT
    subject_id,
    hadm_id,
    MIN(starttime) AS first_antibiotic_time
  FROM `physionet-data.mimiciv_3_1_derived.antibiotic`
  GROUP BY subject_id, hadm_id
)

-- 3️⃣ Filter only for your cohort and join
SELECT
  ac.hadm_id,
  ac.subject_id,
  fa.first_antibiotic_time
FROM adult_cohort ac
JOIN first_abx fa
  ON ac.hadm_id = fa.hadm_id
ORDER BY ac.hadm_id;
